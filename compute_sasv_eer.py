...
ASV_PREDS_FILE = 'sasv_preds.json'
CM_PREDS_FILE = 'cm_preds.json'
KEYS_FILE =  'sasv_keys.json'
with open(os.path.join(DATA_DIR, ASV_PREDS_FILE), 'r') as f:
    asv_preds = json.load(f)
with open(os.path.join(DATA_DIR, CM_PREDS_FILE), 'r') as f:
    cm_preds = json.load(f)
with open(os.path.join(DATA_DIR, KEYS_FILE),'r') as f:
    keys = json.load(f)
...
print("sv eer is %f"%(sv_eer*100))
print("spf eer is %f"%(spf_eer*100))
print("sasv eer is %f"%(sasv_eer*100))
